...
// linux command:
pwd - показать путь где мы ls - показать содержимое каталога где мы mkdir - создать каталог(папка) rmdir - удалить пустой каталог(папку) touch - создать файл rm - удалить файл rm - rf - удалить все cd - перейти в каталог(папку).- текущий каталог..- каталог выше code.- открыть vscode из текущего каталога cd..- перейти на каталог выше(назад) / -абсолютный путь(путь из системных файлов).cat - вывести содержимое файла в терминал